...
- `Break.AndStop` — breaks the task AND propagates the break to the caller task.
...
### `TimeSlicedFlow`
Like `TimeBoundFlow` but wraps `nextIndex` back to 0 when the time slice expires, enabling round-robin time-slicing across many tasks.
- Constructor: `(float maxMilliseconds)`.

**When to use:** Fair time-slicing across many tasks. Each task gets a slice of time per tick, then the runner moves to the next.
...
using (var collection = new MultiThreadedParallelJobCollection<TestJob>("test", threadCount: 4, immediateStart: false)) {
...
- Constructor: `(name, threadCount, immediateStart)`. If `immediateStart: false`, call `RunOn` or `Complete` to start.
- `onComplete` event fires when all tasks done.
- `Dispose()` disposes ALL added tasks (even if never started).

### MultiThreadedParallelTaskCollection
- Constructor: `(name, threadCount, immediateStart)`.
