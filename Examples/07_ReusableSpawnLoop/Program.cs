...
namespace ReusableSpawnLoop
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            RunBlock(block2);
            Console.WriteLine($"  └─ Break.It hit → block RETURNED again (no allocation!)");
            ...
            Console.WriteLine("  │  (state machine stays alive, no GC alloc)      │");
            Console.WriteLine("  └────────────────────────────────────────────────┘");
            Console.WriteLine();
        ...
        private static void RunBlock(PooledIteratorBlock<SpawnData> block)
        {
        ...
        private static int _spawnRow = 11;

        private static void PrintSpawn(SpawnData data)
        {
        ...
        private static void Animate(SpawnData data)
        {
            SafeSetCursor(0, _spawnRow);
            Console.Write("  │    ");
            var frames = new[] { "▁", "▃", "▅", "▇", "█", "▇", "▅", "▃" };
        ...
        private static void SpinWait(int frames, string label, string icon)
        {
            var spin = new[] { '|', '/', '─', '\\' };
