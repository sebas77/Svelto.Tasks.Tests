...
5. After 800ms, `Task.Delay` completes. The next `Step()` continues the async method, setting `_responseReceived = true`.
6. The main loop exits, having visibly stepped the runner N times.
