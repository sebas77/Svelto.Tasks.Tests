...
20 self-contained console examples, one per main feature. Each folder is an independent project you can run with `dotnet run`.
...
Each example targets `net9.0` (rolls forward to `net10.0` if 9 isn't installed) and references Svelto.Tasks + Svelto.Common via project references — no NuGet packages needed.
...
| 12 | [`12_FrameBudgetTimeBound`](12_FrameBudgetTimeBound/README.md) | TimeBoundFlow | Process tasks for at most 5ms per frame |
...
- [AI Guide: Svelto.Tasks](../AI_GUIDE_Svelto.Tasks.md) — Full API reference
- [AI Guide: Svelto.Common](../AI_GUIDE_Svelto.Common.md) — Data structures and utilities