...
- Tasks must be **thread-safe** in any shared state they access (use `Interlocked` or
  `volatile`).
- The `IParallelTask` for the ExtraLean collection must be a **class** (reference type)
  since the non-generic `MultiThreadedParallelTaskCollection` uses `IParallelTask` as
  the type constraint (which is a reference type interface).