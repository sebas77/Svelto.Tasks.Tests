...
class Program
{
    ...
    static IEnumerator<TaskContract> Traverse(TreeNode node)
    {
        ...
        Console.WriteLine("  └────────────────────────────────────────────");
        PrintTrace(node.Depth, string.Format("ENTERING {0}", node.Name));
        Thread.Sleep(300);
        ...
        foreach (var child in node.Children)
        {
            ...
            _activeNode = node.Name;
            PrintTrace(node.Depth, string.Format("back in {0}", node.Name));
            Thread.Sleep(150);
        }

        PrintTrace(node.Depth, string.Format("EXITING {0}", node.Name));
        Thread.Sleep(200);
    }
    ...
    static void Main()
    {
        ...
        Console.WriteLine("  must wait. With SerialFlow, a root task CANNOT wait for");
        Console.WriteLine("  another root task — use .Continue() instead.");
        Console.WriteLine();
