    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            for (int step = 0; step < totalSteps; step++)
            {
                ...
                DrawStep(step, (float)sw.Elapsed.TotalMilliseconds);
                Thread.Sleep(400);
            }
            ...
            for (int i = 0; i < NumTasks; i++)
            {
                ...
                Console.WriteLine("    Task-{0:D2}: {1} {2,2}× ({3}ms each)",
        ...
        static void DrawStep(int step, float actualMs)
        {
            SafeSetCursor(0, 9);
            ...
            Console.WriteLine("  │  STEP {0,2}/{1}  {2}  TimeBoundFlow({3}ms)  ~{4}ms/task      │  ",
                step + 1, 8, spinner, (int)BudgetMs, TaskMs);
            Console.WriteLine("  ├──────────────────────────────────────────────────────┤  ");
        ...
        static void PrintBanner()
        {
            ...
            Console.WriteLine("  ╚══════════════════════════════════════════════════════════════╝");
            Console.WriteLine();
            Console.WriteLine("  ⚙️ = ran this step   · = deferred (budget exhausted)");
            Console.WriteLine();
        }
    }
}