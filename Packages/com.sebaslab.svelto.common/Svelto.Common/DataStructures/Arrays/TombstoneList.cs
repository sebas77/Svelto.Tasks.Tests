...
namespace Svelto.DataStructures.Experimental
{
    ...
    // Stable-index list with O(1) add/remove via tombstones (freelist); use when you must keep indices valid across removals.
    public class TombstoneList<T>
    {
        ...
        public void Clear()
        {
            ...
            // rebuild freelist (base-1 indices); NextUnusedIndex==0 means "used"
            for (uint i = 0; i < (uint)_buffer.Length; i++)
                _buffer[i].NextUnusedIndex = i + 2;
